...
public:
	...
	LargeSpiralRegionSolver(const std::vector<int> &areas) : m_areas(areas), min_product(INT_MAX) {
...
private:
	...
	//
	void populate_full_grid(std::vector<bool> &full_grid, const int & areas_side_len, const std::vector<bool> &grid) const {
	...
	//
	void shade_cells(std::vector<bool> &grid, const int &shaded_per_area, const int &areas_side_len) {
		...
		for (int i3_1 = 0; i3_1 < v3rem3.size() - 2; ++i3_1) {
			for (int i3_2 = i3_1 + 1; i3_2 < v3rem3.size() - 1; ++i3_2) {
				for (int i3_3 = i3_2 + 1; i3_3 < v3rem3.size(); ++i3_3) {
					...
					for (const int& i5 : v5rem1) {
						...
						if ((grid[1] + grid[2] + grid[8] + grid[9] + grid[16] + grid[17] == shaded_per_area) &&
							...
							for (int i7_1 = 0; i7_1 < v7rem2.size() - 1; ++i7_1) {
								for (int i7_2 = i7_1 + 1; i7_2 < v7rem2.size(); ++i7_2) {
									grid[v7rem2[i7_1]] = false;
									grid[v7rem2[i7_2]] = false;

									for (int i6_1poss = 0; i6_1poss < (grid[38] ? v6rem1poss.size() : 0); ++i6_1poss) {
																	...
																	if (grid[22] + grid[23] + grid[24] + grid[30] + grid[31] + grid[32] + grid[38] + grid[27] == shaded_per_area) {
																		...
																		if (grid[4] + grid[10] + grid[11] + grid[17] + grid[18] + grid[20] == shaded_per_area) {
																			...
																			if ((2 * grid[32]) + (2 * grid[33]) + (2 * grid[34]) == (shaded_per_area - 1)) { // Reserve 1 for centre cell
																				...
																				if (is_shaded_connected(full_grid) && is_shaded_count_equal_between_areas(full_grid, shaded_per_area)) {
